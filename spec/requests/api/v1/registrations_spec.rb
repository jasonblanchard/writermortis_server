  ...
  describe '#create' do
    describe 'when there is a validation error' do
      it 'returns the errors and a status code' do
        post '/api/v1/users', {:user => {:email => user.email, :username => "baduser", :password => 'testpass', :password_confirmation => 'testpass'}}
    ...
    describe "when the password and password validation don't match" do
      it 'returns the errors and a status code' do
        post '/api/v1/users', {:user => {:email => 'newuser@example.com', :username => "newuser", :password => 'testpass', :password_confirmation => 'testpazzzzzz'}}
    ...
    describe 'for a valid request' do
      it 'returns the user object and a 200 status code' do
        post '/api/v1/users', {:user => {:email => 'newuser@example.com', :username => "newuser", :password => 'testpass', :password_confirmation => 'testpass'}}
